...
class EmpleadoSensitiveDataReadIntegrationTest extends AbstractIntegrationTest {
    ...
    void setupUser() {
        empleadoRepository.deleteAll();
                ...
                departamento.setNombre("Read");
                departamento = departamentoRepository.save(departamento);
                departamentoClave = departamento.getClave();

        Empleado auth = new Empleado();
        auth.setNombre("reader");
        auth.setEmail("reader@example.com");
    ...
    @Test
    void shouldNotExposeContrasenaInReadEndpoints() throws Exception {
        ...
        MvcResult create = mockMvc.perform(post("/api/v1/empleados")
                        .with(httpBasic("reader@example.com", "Abcdef1!"))
        ...
        mockMvc.perform(get("/api/v1/empleados/{clave}", clave).with(httpBasic("reader@example.com", "Abcdef1!")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.contrasena").doesNotExist());

        mockMvc.perform(get("/api/v1/empleados").param("size", "5").with(httpBasic("reader@example.com", "Abcdef1!")))
                .andExpect(status().isOk())
