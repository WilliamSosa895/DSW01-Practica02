...
class DepartamentoPaginationIntegrationTest extends AbstractIntegrationTest {
    ...
    void setupData() {
        ...
        auth.setNombre("deppager");
        auth.setEmail("deppager@example.com");
    ...
    @Test
    void shouldEnforceFixedPaginationSizeForDepartments() throws Exception {
        mockMvc.perform(get("/api/v1/departamentos").with(httpBasic("deppager@example.com", "Abcdef1!")).param("page", "0").param("size", "5"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.size").value(5));

        mockMvc.perform(get("/api/v1/departamentos").with(httpBasic("deppager@example.com", "Abcdef1!")).param("size", "4"))
